...
- [x] R31: 卡片双击翻面「色彩档案」(3D 弹簧翻转;背面 = 标题 + AI 一句话色彩故事(首翻懒生成、持久化到 card.story)+ 六行 hex/RGB 点按复制;FPC_FLIP 钩子)+ 设置切模式落在该模式的卡上(每模式记住最后浏览,否则最新一张)。坑:AI 401 = 开源洗掉默认 key 后模拟器要带 SIMCTL_CHILD_FPC_AI_KEY;英文 story 15~30 词会超 120 字符清洗上限(已放宽 220);AppInfo.plist 加 ATS NSAllowsLocalNetworking 供局域网 AI 端点
